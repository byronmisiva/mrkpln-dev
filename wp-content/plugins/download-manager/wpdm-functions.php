...
/**
 * @usage Check multi user status ! This functions deprecated from wpdm pro 4.0.0
 * @param string $cond
 * @return bool|string
 */
...
function wpdm_allowed_roles($id){
    ...
    if(is_array($cats)){
	foreach($cats as $cat){
		$croles = WPDM_CategoryHandler::GetAllowedRoles($cat->term_id);
...
function wpdm_user_has_access($id, $type = 'package'){
	...
	else $roles = WPDM_CategoryHandler::GetAllowedRoles($id);
...
//    }
    ...
    '.__('Enter Correct Password to Download','wpdmpro').'
  </div>
  <div class="panel-body">
        <div id="msg_' . $package['ID'] . '" style="display:none;">'.__('Processing...','wpdmpro').'</div>
        ...
        ';
            ...
            $data .= '
        <input type="password"  class="form-control" placeholder="'.__('Enter Password','wpdmpro').'" size="10" id="password_' . $unqid . '_' . $package['ID'] . '" name="password" />
...
function wpdm_email_lock_form($package)
{
    ...
    if (isset($package['email_lock']) && $package['email_lock'] == '1') {
        ...
        $blabel = isset($package['email_heading']) ? $package['email_heading'] : __('Subscribe', 'wpdmpro');
  ...
  <div class="panel-body">
        ...
        <button id="wpdm_submit_' . $unqid . '_' . $package['ID'] . '" class="wpdm_submit btn btn-success  group-item"  type=submit>'.$blabel.'</button>
      </span>
    </div>
...
function wpdm_tweet_button($package, $buttononly = false)
{
    ...
    if (isset($_COOKIE[$var]) && $_COOKIE[$var] == 1)
        ...
        $data = '<div id="tweet_content_' . $package['ID'] . '" class="locked_ct pull-left"><a href="https://twitter.com/share?text=' . $tweet_message . '" class="twitter-share-button" data-via="w3eden">Tweet</a></div><div style="clear:both"></div>';
...
function wpdm_edit_package_form(){

    if(is_user_logged_in())
        include('tpls/wpdm-be-member.php');
    else
        include('tpls/wpdm-add-new-file-front.php');
}


function wpdm_locate_template(){
...
function wpdm_emails()
{
    if (isset($_GET['task']) && $_GET['task'] == 'template')
        include("admin/wpdm-emails-template.php");
    else
        include("admin/wpdm-emails.php");
}
...
function wpdm_get_packages($args = array())
{
    ...
    $items_per_page = isset($items_per_page) && $items_per_page>0 ? $items_per_page : 9;
...
function wpdm_package_file_list($file)
{
    ...
    if (count($file['files']) > 0) {
      ...
      //  if ($idvdl && ($pwdlock || !$olock)) { $dlcol = "<th align=center>".__("Action","wpdmpro")."</th>"; $swl = 1; }
        if ($idvdl && ($pwdlock || !$olock)) { $dlcol = ""; $swl = 1; }
       ...
       // $fhtml = "<audio controls autoplay preload='auto' style='width: 100%;display: none' src='#' id='audio-player-{$file['ID']}'></audio><table class='wpdm-filelist table table-hover'><thead><tr><th>".__("File","wpdmpro")."</th>{$pwdcol}{$dlcol}</tr></thead><tbody>";
        $fhtml = "<audio controls autoplay preload='auto' style='width: 100%;display: none' src='#' id='audio-player-{$file['ID']}'></audio><table class='wpdm-filelist table table-hover'> <tbody>";
        if (is_array($allfiles)) {
            ...
            foreach ($allfiles as $ind => $sfile) {
                ...
                if ($swl) {
                    ...
                    $mp3 = end($mp3);
                    $mp3 = strtolower($mp3);
                    $play = $mp3 == 'mp3'?"<a rel='noindex nofollow' class='btn btn-success btn-xs wpdm-btn-play' href='#' data-player='audio-player-{$file['ID']}' data-song='" . wpdm_download_url($file) . "&ind=" . $ind . "'><i style='opacity:0.5;margin-top:0px' class='fa fa-play'></i></a>":"";
                    ...
                    if ($fileinfo[$sfile]['password'] != '' && $pwdlock)
                        $fhtml .= "<td width='110' align=right><input  onkeypress='jQuery(this).removeClass(\"input-error\");' size=10 type='password' value='' id='pass_{$file['ID']}_{$ind}' placeholder='".__('Password','wpdmpro')."' name='pass' class='form-control input-sm inddlps' /></td>";
                    if ($fileinfo[$sfile]['password'] != '' && $pwdlock)
                        ...
                        $fhtml .= "<td width=300 align=center>
                                                    <div   class='btn btn-primary btn-xs btn-sendemail'  ttl='". $ttl ."'  href='" . wpdm_download_url($file) . "&ind=" . $ind . "'><i style='opacity:0.5;margin-top:0px' class='fa fa-download'></i>&nbsp;".__("Send Email","wpdmpro")."</div>
                                                    <a rel='noindex nofollow' class='btn btn-primary btn-xs' href='" . wpdm_download_url($file) . "&ind=" . $ind . "'><i style='opacity:0.5;margin-top:0px' class='fa fa-download'></i>&nbsp;".__("Download","wpdmpro")."</a>&nbsp;{$play}</td></tr>";
                } else {
            ...
            foreach ($dfiles as $ind => $sfile) {
                ...
                if ($idvdl == 1 && ($pwdlock || !$olock)) {
                    ...
                    if ($fileinfo[$sfile]['password'] != '' && $pwdlock)
                        $fhtml .= "<td width='110' align=right><input  onkeypress='jQuery(this).removeClass(\"input-error\");' size=10 type='password' value='' id='pass_{$file['ID']}_{$ind}' placeholder='".__('Password','wpdmpro')."' name='pass' class='form-control input-sm inddlps' /></td>";
                    if ($fileinfo[$sfile]['password'] != '' && $pwdlock)
                        $fhtml .= "<td width=150><button class='inddl btn btn-primary btn-sm' file='{$sfile}' rel='" . wpdm_download_url($file) . "&ind=" . $ind . "' pass='#pass_{$file['ID']}_{$ind}'><i class='fa fa-download'></i>&nbsp;Download</button></td></tr>";
                    else
                        $fhtml .= "<td width=150 align=center><a rel='noindex nofollow' class='btn btn-primary btn-xs' href='" . wpdm_download_url($file) . "&ind=" . $ind . "'><i style='opacity:0.5;margin-top:0px' class='fa fa-download'></i>&nbsp;Download</a></td></tr>";
                } else {
                    $ttl = isset($fileinfo[$sfile]['title']) && $fileinfo[$sfile]['title']!="" ? $fileinfo[$sfile]['title'] :  str_replace('/', " <i class='fa fa-angle-right text-primary'></i> ",str_replace($dir, "", $sfile));
                    $fhtml .= "<tr><td>{$ttl}</td></tr>";
                }
 ...
 */
function wpdm_package_file_list_alt($file)
{
    global $current_user;
    $file['files'] = maybe_unserialize($file['files']);
    $fhtml = '';
        ...
        if (is_array($allfiles)) {
            ...
            foreach ($allfiles as $ind => $sfile) {
                ...
                if(!isset($fileinfo[$sfile]['password'])) $fileinfo[$sfile]['password'] = "";
                    ...
                    $fhtml .= "<div class='panel-heading'>{$ttl}</div>";
                    ...
                    if($thumb)
                        ...
                        $fhtml .= "<div class='panel-body text-center'><img class='file-ico' src='".WPDM_BASE_URL.'file-type-icons/'.$ext.'.png'."' alt='{$ttl}' /></div><div class='panel-footer'>";
...
function wpdm_option_field($data) {
    switch($data['type']):
        ...
        case 'dropdown':
            ...
            break;
        case 'notice':
            return "<div class='alert alert-info' style='margin: 0'>$data[notice]</div>";
        case 'textarea':
...
function wpdm_custom_data($pid)
{
    ...
    return apply_filters('wpdm_custom_data',$data, $pid);
}
...
function wpdm_setup_package_data($vars)
{
    ...
    $vars['file_list_email'] = wpdm_package_file_list($vars);
    ...
    if (!isset($vars['icon']) || $vars['icon'] == '') {
        ...
        $vars['icon'] = '<img  alt="'.$vars['title'].'" class="wpdm_icon"   src="' . plugins_url($vars['icon']) . '" />';
    else if (!strpos($vars['icon'], ">"))
...
function wpdm_doc_preview($package){
        ...
        foreach($files as $i=>$sfile){
            $ifile = $sfile;
            $sfile = explode(".", $sfile);
            if(in_array(end($sfile),array('pdf','doc','docs','xls','xlsx','ppt','pptx'))) { $ind = WPDM_Crypt::Encrypt($ifile); break; }
        }
        if($ind==-1) return "";
...
function wpdm_audio_playlist($package, $return  = false )
{
    ...
    if(count($audios)>0){
        ...
        foreach($audios as $index => $audio){
            $index = WPDM_Crypt::Encrypt($audio);
            $song = home_url("/?wpdmdl={$package['ID']}&_nonce={$nonce}&ind={$index}&play=". urlencode(basename($audio)));
...
function wpdm_enqueue_scripts()
{
    ...
    if(is_object($post) && has_shortcode($post->post_content,'wpdm_frontend')){
    ...
    //wp_enqueue_style('choosen-css', plugins_url('/download-manager/css/chosen.css'), 999999);
...
{
    require_once(dirname(__FILE__) . '/tpls/wpdm-dashboard.php');
}
...
function wpdm_columns_th($defaults) {
    ...
    $img['image'] = "<span class='fa-stack' style='font-size: 0.8em'><i class='fa fa-circle-thin fa-stack-2x'></i><i  style='font-size: 80%' class='fa fa-image fa-stack-1x'></i></span>";
    wpdm_array_splice_assoc( $defaults, 1, 0, $img );
    $otf['download_count'] = "<span class='fa-stack' style='font-size: 0.8em'><i class='fa fa-circle-thin fa-stack-2x'></i><i  style='font-size: 80%' class='fa fa-download fa-stack-1x'></i></span>";
...
// Add with main RSS feed
function wpdm_rssfeed($query) {
    if ( isset($query['feed'])  && !isset($query['post_type']) && get_option('__wpdm_rss_feed_main', 1) == 1 ){
        $query['post_type'] = is_array($query['post_type'])? array_merge($query['post_type'], array('post','wpdmpro')):array('post','wpdmpro');
    }
    return $query;
}

// Schedule custom ping
function wpdm_custom_pings($post_id){
    wp_schedule_single_event(time(), 'do_pings', array($post_id));
}

function wpdm_tpl_path($file){
