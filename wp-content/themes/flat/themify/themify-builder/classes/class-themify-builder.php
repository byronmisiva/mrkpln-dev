	...
	class Themify_Builder {
		...
		function module_lightbox_options_ajaxify() {
			...
			if ( false !== $module ) {
				...
				echo sprintf( '<p>%s</p>', __( 'Module ' . $module_name . ' is not active', 'themify' ) );
			}
		...
		function get_template_row( $rows, $row, $builder_id, $echo = false ) {
			...
			if( ! isset( $row['styling']['background_type'] ) && '' != $row['styling']['background_video'] ) {
			...
			<div class="<?php echo implode( ' ', $row_classes ); ?>" <?php echo $data_video; ?>>
				...
				<div class="row_inner">
					...
					<?php
					// Background Slider
					if ( isset( $row['styling']['background_slider'] ) && ! empty( $row['styling']['background_slider'] ) ) :
